...
impl WebGL2RenderingContext {
    ...
    fn check_uniform_vec_section(
        &self,
        offset: u32,
        length: u32,
        array_size: usize,
    ) -> WebGLResult<(usize, usize)> {
...
impl WebGL2RenderingContextMethods for WebGL2RenderingContext {
    ...
    fn Uniform1uiv(
        ...
        self.base.with_location(location, |location| {
            ...
            let (offset, length) =
                self.check_uniform_vec_section(src_offset, src_length, val.len())?;
            let val = val[offset..offset + length].to_vec();
            if val.is_empty() {
                return Err(InvalidValue);
            }
